...
// جلب المنتجات مع التصفية حسب البحث أو السعر
$sql = "SELECT * FROM products WHERE category = 'phones' 
        AND name LIKE ? 
        AND price BETWEEN ? AND ?";
$stmt = $conn->prepare($sql);
$search_term = "%$search%";
$stmt->bind_param("sii", $search_term, $min_price, $max_price);
$stmt->execute();
...
<head>
  <meta charset="UTF-8" />
  <title>الهواتف - السعر البسيط</title>
...
🔍 يمكنك البحث عن هاتف حسب <strong>الاسم أو الماركة</strong>، وتحديد <strong>ميزانيتك</strong> (أدنى وأعلى سعر) لاختيار الهواتف التي تناسبك.
</p>
<form method="GET" style="text-align: center; margin-bottom: 30px;">
  ...
  <input type="number" name="min_price" placeholder="💰 من" value="<?php echo $min_price; ?>" style="padding: 8px; width: 100px;">

  <input type="number" name="max_price" placeholder="💰 إلى" value="<?php echo $max_price; ?>" style="padding: 8px; width: 100px;">
  ...
  <a href="phones.php" style="margin-right: 15px; color: #070707ff; text-decoration:none;">🔄 إعادة تعيين</a>
  ...
  <div class="product-grid">
    ...
    if ($result && $result->num_rows > 0) {
      while($row = $result->fetch_assoc()) {
        echo '
        <div class="product">
          <img src="admin/image/phones/' . htmlspecialchars($row["image"]) . '" alt="' . htmlspecialchars($row["name"]) . '">
      ...
      echo "<p style='text-align:center;'>لا توجد منتجات حاليا في فئة الهواتف.</p>";
    }
